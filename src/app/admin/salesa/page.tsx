...
import SalesaClient from "./SalesaClient";

export const dynamic = "force-dynamic";
export const metadata: Metadata = { title: "SALESA · วิเคราะห์ยอดขายรายวัน" };

export default function SalesaPage() {
  ...
  if (branchId == null || !isSalesaBranch(branchId)) {
    return (
      <div className="space-y-4">
        <h1 className="text-2xl font-bold text-slate-800">SALESA · วิเคราะห์ยอดขายรายวัน</h1>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-2 flex-wrap">
        ...
        <Link href="/admin/salesa/settings" className="btn-secondary text-sm">⚙️ ตั้งค่ากลุ่ม LINE</Link>
      </div>
      <SalesaClient branchName={branch?.name ?? `#${branchId}`} />
    </div>
  );
}
