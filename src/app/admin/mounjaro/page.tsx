...
import { listMyPatientsEnriched, listPendingIntake, listPendingInvitations, type MjActor } from "@/lib/mounjaro-db";
...
export default function ClinicalListPage() {
  ...
  if (!isClinicalUnlocked(user)) {
    return <ClinicalClient mode="locked" patients={[]} pending={[]} pendingInvitations={[]} />;
  }
  const patients = listMyPatientsEnriched(user as MjActor);
  const pending = listPendingIntake(user as MjActor);
  const pendingInvitations = listPendingInvitations(user as MjActor);
  return <ClinicalClient mode="list" patients={patients} pending={pending} pendingInvitations={pendingInvitations} />;
}
