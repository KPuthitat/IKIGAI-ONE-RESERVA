...
import type { BreakSkipWithName } from "@/lib/break-skip";
...
export default function BreakSkipReviewClient({ rows }: { rows: BreakSkipWithName[] }) {
  ...
  async function decide(id: number, action: "approve" | "reject") {
    ...
    if (action === "reject") note = window.prompt("เหตุผลที่ไม่อนุมัติ (ถ้ามี):") ?? "";
    setBusyId(id); setErr(null);
    try {
      const res = await fetch(apiUrl(`/api/admin/persona/break-skip-requests/${id}`), {
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-xs text-slate-500 border-b border-slate-200">
                ...
                <th className="py-2 pr-3">เวลาพัก</th>
            ...
            <tbody>
              {pending.map((r) => (
                <tr key={r.id} className="border-b border-slate-100 last:border-0 align-top">
                  ...
                  <td className="py-2 pr-3 text-slate-600 max-w-[260px]">{r.reason ?? "—"}</td>
            ...
            <tbody>
              {decided.map((r) => (
                <tr key={r.id} className="border-b border-slate-100 last:border-0 align-top">
                  ...
                  <td className="py-1.5 pr-3 whitespace-nowrap tabular-nums text-slate-500">{r.break_label ?? "—"}</td>
                  <td className="py-1.5 pr-3 text-slate-500 max-w-[240px]">
                    {r.reason ?? "—"}
