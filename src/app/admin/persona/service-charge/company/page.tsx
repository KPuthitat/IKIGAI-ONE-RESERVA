...
export default function CompanyServiceChargePage({
  ...
  // Group by ประเภทพนักงาน × โหมดภาษี (owner 2026-09-03: แยกให้เห็นสัดส่วน —
  // พนักงานประจำ ในระบบ/นอกระบบ, พาร์ทไทม์). Each is a labelled section so the
  // SSO-vs-WHT split (who has 3% withheld from SVC) is obvious. PT is uniformly
  // หัก ณ ที่จ่าย. Empty sections are dropped.
  const isOther = (ty: string | null) => ty !== "ft" && ty !== "pt";
  const distGroups = [
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="card">
        ...
        {shared && (
          <div className="text-[11px] text-brand mt-2 font-medium">
            คำนวณรวมทั้งบริษัทเปิดอยู่ — คนที่ถูกส่งไปสาขาที่ยังไม่มียอด ถูกดึงเข้าไปรับส่วนแบ่งจากสาขาที่มียอด
          </div>
        )}
      ...
      <div className="card text-xs text-slate-500 space-y-1">
        ...
        <div>· คำนวณรวมทั้งบริษัท (ถ้าเปิด): คนที่ทำสาขาที่มียอด ได้ของสาขาตัวเอง — เฉพาะคนที่ถูกส่งไปสาขายังไม่มียอด ถึงถูกดึงเข้าไปรับส่วนแบ่งจากสาขาที่มียอด</div>
