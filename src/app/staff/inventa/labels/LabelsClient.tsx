...
// QR codes are generated server-side at /api/inventa/qr (owner R2
// 2026-06-07). The old client-side CDN loader (window.QRCode) failed
// silently whenever the CDN was blocked/offline; a same-origin <img>
// always renders, prints, and downloads.
function qrSrc(code: string, size = 220): string {
          ...
          {chosen.map((i) => {
            ...
            return (
              <div key={i.id}
                ...
                <img src={qrSrc(code)} alt={code} className="w-28 h-28" />
                ...
                <a href={qrSrc(code, 512)} download={`qr-${code}.png`}
                  className="no-print mt-1 text-[10px] text-brand hover:underline">
                  ดาวน์โหลด PNG
                </a>
              </div>
            );
