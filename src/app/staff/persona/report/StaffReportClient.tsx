...
export default function StaffReportClient({
  ...
  async function save() {
    ...
    try {
      const res = await fetch(apiUrl("/api/persona/manager-report"), {
        method: "POST",
  ...
  async function del(id: number) {
    ...
    const res = await fetch(apiUrl(`/api/persona/manager-report/${id}`), { method: "DELETE" });
    ...
    <div className="space-y-5">
      <div className="card space-y-3">
        ...
        <Field label="สถานการณ์ / ปัญหา / เหตุการณ์วันนี้" value={situation} onChange={setSituation}
          placeholder="เช่น แอร์โซนสองเสีย · ของหมดสต๊อก 2 รายการ · ลูกค้าคอมเพลนคิวช้า" />
        <Field label="เรื่องที่อยากเสนอเข้าประชุมประจำสัปดาห์" value={topics} onChange={setTopics}
          placeholder="เช่น ขอเพิ่มคนช่วงเสาร์-อาทิตย์ · เสนอปรับเมนู · ขอซื้ออุปกรณ์" />
      ...
      <div>
        <h2 className="text-sm font-bold text-slate-700 mb-2">รายงานที่ฉันส่ง (14 วัน)</h2>
        <div className="space-y-2">
          {myReports.length === 0 ? (
            <div className="card text-sm text-slate-400 text-center py-6">ยังไม่มีรายงาน</div>
          ) : myReports.map((r) => (
