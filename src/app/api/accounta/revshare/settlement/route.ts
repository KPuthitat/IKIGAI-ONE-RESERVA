...
const MonthKeyZ = z.string().regex(/^\d{4}-\d{2}$/);
const PostZ = z.object({
  partner: z.number().int().positive(),
  year: z.number().int(),
  month: z.number().int().min(1).max(12),
  months: z.array(MonthKeyZ).max(12).optional(),
...
/** Comma-separated "YYYY-MM" list from the query string → validated array (or
 *  undefined when absent, so preview falls back to the stored/anchor set). */
function parseMonths(raw: string | null): string[] | undefined {
  if (!raw) return undefined;
  const ks = raw.split(",").map((s) => s.trim()).filter((s) => /^\d{4}-\d{2}$/.test(s));
  return ks.length ? ks.slice(0, 12) : undefined;
}
...
export function GET(req: Request) {
  ...
  const months = parseMonths(sp.get("months"));
  if (![partner, year, month].every(Number.isInteger)) return NextResponse.json({ error: "bad_params" }, { status: 400 });
  const preview = previewSettlement(partner, branchId!, year, month, months);
...
export async function POST(req: Request) {
  ...
  let done = false;
  if (action === "save") done = !!saveSettlement(partner, branchId!, year, month, months);
  else if (action === "issue") done = issueSettlement(partner, branchId!, year, month, invoice_no ?? null, bkkNowIso(), months);
