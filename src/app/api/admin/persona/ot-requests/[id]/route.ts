...
// Supervisor/admin approves or rejects an OT request. Optionally edits
// the requested-until time — because that changes a pay-affecting value,
// editing requires the admin's PIN (owner 2026-06-03). Approve/reject
// alone does not.
...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  ...
  const row = db.prepare(
    "SELECT id, requested_until FROM ot_requests WHERE id = ?"
  ).get(id) as { id: number; requested_until: string } | undefined;
  if (!row) return NextResponse.json({ error: "not_found" }, { status: 404 });

  // Editing the requested time is a pay change → require PIN.
  ...
  db.prepare(`
    ...
    WHERE id = ?
  `).run(d.action === "approve" ? "approved" : "rejected", user.id, new Date().toISOString(), id);
