...
export async function PATCH(
  ...
  try {
    db.transaction(() => {
      if (clockIn === null && clockOut === null) {
        // Clear the override → revert this day to the time-clock data.
        db.prepare(`
        ...
        db.prepare(`
          INSERT INTO payroll_line_days
            (period_id, user_id, work_date, clock_in, clock_out, edited_by, edited_at)
          VALUES (?, ?, ?, ?, ?, ?, ?)
        ...
        `).run(periodId, userId, d.work_date, clockIn, clockOut, user.id, new Date().toISOString());
      }
  ...
  `).run(
    ...
    JSON.stringify({ work_date: d.work_date, clock_in: clockIn, clock_out: clockOut, line: after ?? {} }),
    `แก้เวลาวันที่ ${d.work_date}`,
