...
export default function BookingsClient({
  ...
  async function setStatus(id: number, status: Row["status"]) {
    ...
    if (!res.ok) {
      ...
      const msg = j.error === "no_table_assigned"
        ? t("admin.bookings.err.noTableAssigned")
        : t("admin.bookings.errorGeneric");
      alert({
  ...
  async function confirmPending(id: number) {
    const ok = await confirm({
      title: t("admin.bookings.confirmPendingTitle"),
      body: <p>{t("admin.bookings.confirmPendingBody")}</p>,
      confirmLabel: t("admin.bookings.btn.confirmAndSend"),
  ...
  return (
    ...
    <div className="space-y-3">
      ...
      {(() => {
        ...
        return (
          <div className="rounded-xl border-l-4 border-amber-400 bg-amber-50 px-3.5 py-2.5 text-sm text-amber-900">
            <span className="font-semibold">⚠ {t("admin.bookings.pendingBanner.title", { n: pendingCount })}</span>
            <span className="ml-2 text-amber-800">{t("admin.bookings.pendingBanner.body")}</span>
          </div>
        );
      })()}
      {bookings.map((b) => (
        <div
          ...
          className={
            ...
            (b.status === "pending" ? "border-l-4 border-amber-400 bg-amber-50/40 " : "")
          }
        >
              ...
              {b.status === "pending" && (
                <>
                  <button
                    ...
                    disabled={busyId === b.id || b.table_id == null}
                    className="btn-primary"
                    title={b.table_id == null ? t("admin.bookings.err.noTableAssigned") : undefined}
                  >{t("admin.bookings.btn.confirmAndSend")}</button>
                  <button
