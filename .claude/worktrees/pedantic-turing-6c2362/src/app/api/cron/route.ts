...
import { notifyCustomer, notifyStaff, notifyStaffPendingOverdue } from "@/lib/line";
...
export async function POST(req: Request) {
  ...
  // แจ้งเตือนแอดมิน: pending booking ที่เลยเวลาจองไปแล้ว 30 นาทีโดย admin
  // ยังไม่ได้กำหนดโต๊ะ + ยืนยัน → ส่ง LINE text ไปยัง staff ที่ผูกไว้กับสาขา
  // ส่งครั้งเดียวต่อ booking (กันยิงซ้ำด้วย notification_log)
  let pendingOverdueAlerts = 0;
  const overduePending = db.prepare(`
    SELECT b.* FROM bookings b
    WHERE b.status = 'pending'
      AND NOT EXISTS (
        SELECT 1 FROM notification_log nl
        WHERE nl.booking_id = b.id AND nl.type = 'pending_overdue'
      )
  `).all() as Booking[];
  const branchById = new Map(branches.map((br) => [br.id, br]));
  for (const b of overduePending) {
    if (bookingStartMs(b.booking_date, b.booking_time) >= noShowCutoffMs) continue;
    const br = branchById.get(b.branch_id);
    if (!br) continue;
    try {
      await notifyStaffPendingOverdue(br, b);
      pendingOverdueAlerts++;
    } catch (e) {
      console.error("pending overdue alert error", e);
    }
  }
  ...
  return NextResponse.json({
    ...
    pending_overdue_alerts: pendingOverdueAlerts,
