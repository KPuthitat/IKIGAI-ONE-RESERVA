...
export default function BookingForm({
  ...
  async function submitForm(e: React.FormEvent) {
    ...
    if (!form.customer_name.trim() || !form.customer_phone.trim()) {
      // Walk-in tolerates an empty phone, but every other path keeps both required
      if (!(mode === "walkin" && form.customer_name.trim())) {
        setError(t("booking.error.missingNamePhone"));
        return;
      }
    }
    if (isPastTime && mode !== "walkin") {
      setError(t("booking.error.pastTime"));
      return;
    }

    // Admin modes still pick a table here (walk-in/phone). Customer mode
    // sends no table — admin assigns one and confirms after the request
    // arrives. So only admin modes go through the "choose" step.
    const isAdmin = mode === "walkin" || mode === "phone";
    if (isAdmin && chosenTable === null) {
      // Need to fetch suggestions once, then let staff pick
      setSubmitting(true);
      try {
  ...
  async function confirmBooking() {
    ...
    try {
      const isAdmin = mode === "walkin" || mode === "phone";
      const tableId = isAdmin
        ? (chosenTable === "auto" ? suggestions[0].id : chosenTable)
        : null;
      ...
      const body: Record<string, unknown> = isAdmin
        ...
        : {
            ...
            ...form,
            source: form.sources.length > 0 ? JSON.stringify(form.sources) : "",
            // Customer flow leaves table_id off entirely — admin assigns
            // when reviewing the pending request.
            lang   // tag booking with current UI lang so the LINE Flex
                   // card renders in the same language at confirm time
          };
  ...
  if (step === "done" && resultId) {
    return (
      <div className="card text-center">
        ...
        <h2 className="text-xl font-bold mb-2 text-slate-800">{t("booking.pending.title")}</h2>
  ...
  return (
    <form onSubmit={submitForm} className="space-y-4">
      ...
      <button disabled={submitting || isPastTime} className="btn-primary w-full text-base py-3.5">
        {submitting
          ? (mode === "customer" ? t("booking.cta.sending") : t("booking.cta.searching"))
          : (mode === "customer" ? t("booking.cta.sendRequest") : t("booking.cta.findTable"))}
      </button>
    </form>
  );
}
