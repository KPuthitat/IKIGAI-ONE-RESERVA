...
function runMigrations(db: Database.Database): void {
  ...
  // Allow status='pending' on bookings (online customer requests waiting
  // for admin to assign a table + confirm). SQLite has no ALTER CHECK,
  // so recreate the table when the existing CHECK doesn't include 'pending'.
  const bSql = db.prepare(
    "SELECT sql FROM sqlite_master WHERE type='table' AND name='bookings'"
  ).get() as { sql: string } | undefined;
  if (bSql && !bSql.sql.includes("'pending'")) {
    db.exec(`
      BEGIN;
      ...
      INSERT INTO bookings_new
        (id, branch_id, table_id, customer_name, customer_phone, party_size,
         source, customer_origin, is_member, lang, booking_channel, ref_no,
         booking_date, booking_time, duration_minutes, status, notes,
         line_user_id, created_by, created_at, updated_at, seated_at, cancelled_at)
      SELECT id, branch_id, table_id, customer_name, customer_phone, party_size,
             source, customer_origin, is_member, lang, booking_channel, ref_no,
             booking_date, booking_time, duration_minutes, status, notes,
             line_user_id, created_by, created_at, updated_at, seated_at, cancelled_at
      FROM bookings;
      ...
      CREATE INDEX IF NOT EXISTS idx_bookings_ref_no ON bookings(ref_no);
      COMMIT;
    `);
  }
...
export type BookingStatus = "pending" | "confirmed" | "seated" | "no_show" | "cancelled" | "completed";
